...
public class FloatingLayout extends ViewGroup {
    //子视图已存放的位置
    private SparseArray<LinkedList<Rect>> childenPos;// 等效于Map<Integer,LinkedList<Rect>>
    ...
    private int itemHorizontalMargin = 20;
        ...
        public void onClick(View v) {
            if (null != onItemCheckedChangeListener) {
                ...
                if (currposition != position) {
                    v.setSelected(true);
                    if (currposition != -1) {
                        getChildAt(currposition).setSelected(false);
                    }
                    currposition = position;
                }
                onItemCheckedChangeListener.onItemCheckedChange(position);
            } else {
    ...
    public void setDefaultView() {
        if (getChildAt(currposition) != null) {
            getChildAt(currposition).setSelected(true);
        }
    }
